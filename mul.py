...
def generate_fraction(n):
    ...
    powers_numerator = [random.randint(6, 26) for _ in range(len(numerator))]  # генерируем случайные степени для каждого числа в числителе
    powers_denominator = [random.choice([power, power, power-1, power+1, power, power-2]) for power in powers_numerator]  # генерируем случайные степени для каждого числа в знаменателе, такие что отличаются от степеней в числителе не более чем на 2
    ...
    numerator_str = ' * '.join(f"{num}^{power}" for num, power in zip(numbers, powers_numerator))
    denominator_str = ' * '.join(f"{num}^{power}" for num, power in zip(denominator, powers_denominator))
    ...
    numerator = [num ** power for num, power in zip(numerator, new_powers_numerator)]  # возводим каждое число в числителе в соответствующую степень
    denominator = [num ** power for num, power in zip(denominator, new_powers_denominator)]  # возводим каждое число в знаменателе в соответствующую степень
...
n = 5  # количество чисел в числителе и знаменателе
